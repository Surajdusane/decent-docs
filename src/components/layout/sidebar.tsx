...
const itemVariants = cva(
  'relative flex flex-row items-center gap-2 rounded-sm p-2 text-start text-fd-muted-foreground [overflow-wrap:anywhere] md:py-1.5 [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      active: {
    ...
    <Context.Provider value={context}>
      ...
      >
        <div
          {...inner}
          className={cn(
            'flex size-full max-w-[260px] flex-col pt-2 md:ms-auto md:w-(--fd-sidebar-width) md:border-e border-dashed md:pt-4',
            inner?.className,
          )}
        >
          {props.children}
...
export function SidebarFooter(props: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      {...props}
      className={cn(
        'flex flex-col border-t border-dashed px-4 py-3 empty:hidden',
        props.className,
      )}
    >
      {props.children}
